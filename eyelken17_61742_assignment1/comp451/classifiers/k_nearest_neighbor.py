...
class KNearestNeighbor(object):
    ...
    def compute_Cosine_distances_two_loops(self, X):
        ...
        for i in range(num_test):
            for j in range(num_train):
                ...
                #####################################################################
                # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
                dists[i,j] = np.sum(self.X_train[j] * X[i])/(np.sqrt(np.sum(X[i]*2)) * np.sqrt(np.sum(self.X_train[j]*2)))
                # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            return dists
    ...
    def compute_Cosine_distances_one_loop(self, X):
        ...
        for i in range(num_test):
            ...
            #######################################################################
            # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
            dists[i] = np.sum(self.X_train * X[i], axis=1)/(np.sqrt(np.sum(X[i]*2)) * np.sqrt(np.sum(self.X_train*2, axis = 1)))
            # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
    ...
    def compute_Cosine_distances_no_loops(self, X):
        ...
        #########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        dists = np.dot(X, self.X_train.T) / (np.sqrt(np.sum(X**2, axis=1))[:,np.newaxis] * np.sqrt(np.sum(self.X_train**2, axis=1)))
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        return dists
